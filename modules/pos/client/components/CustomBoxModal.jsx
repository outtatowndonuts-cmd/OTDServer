  ...
  return (
    <div className="modal-backdrop" onClick={onCancel}>
      <div className="modal-box" style={{ maxWidth: 600, width: '94vw', maxHeight: '80vh', overflowY: 'auto' }} onClick={(e) => e.stopPropagation()}>
        ...
        <p style={{ color: '#888', marginBottom: 20, fontSize: '0.9rem' }}>Choose a box type to build.</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12 }}>
          {configs.map((cfg) => (
            <button
              ...
              onMouseEnter={(e) => (e.currentTarget.style.borderColor = '#e97320')}
              onMouseLeave={(e) => (e.currentTarget.style.borderColor = '#2a2d3a')}
            >
              <div style={{ fontWeight: 700, fontSize: '1rem', marginBottom: 6 }}>{cfg.name}</div>
              <div style={{ color: '#9ca3af', fontSize: '0.85rem', marginBottom: cfg.discountPct > 0 ? 8 : 0 }}>{cfg.size} items</div>
              {cfg.discountPct > 0 && (
                <div
                  style={{
...
function BoxFiller({ box, products, onConfirm, onBack }) {
  ...
  return (
    <div className="modal-backdrop">
      ...
      >
        {/* Header */}
        <div style={{ padding: '18px 24px 14px', borderBottom: '1px solid #2a2d3a', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <h2 style={{ margin: 0, fontSize: '1.1rem' }}>
              📦 {box.name}
              <span style={{ fontWeight: 400, color: '#9ca3af', fontSize: '0.9rem', marginLeft: 8 }}>{box.size} items</span>
            </h2>
            <button
              onClick={onBack}
              style={{
          ...
          >
            <div
              style={{
                height: '100%',
                width: pct + '%',
                background: boxFull ? '#6ee7b7' : tooMany ? '#f87171' : '#e97320',
                borderRadius: 6,
                transition: 'width .2s',
              }}
            />
          </div>
          <div style={{ fontSize: '0.82rem', color: boxFull ? '#6ee7b7' : tooMany ? '#f87171' : '#9ca3af' }}>{boxFull ? '✓ Box is full — ready to add!' : tooMany ? `Too many — remove ${totalSelected - box.size}` : `${remaining} more item${remaining !== 1 ? 's' : ''} to fill the box`}</div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 24px' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem', marginTop: 12 }}>
            <thead>
              <tr style={{ color: '#9ca3af', fontSize: '0.78rem', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                    ...
                    <tr key={p._id} style={{ borderTop: '1px solid #1e2130' }}>
                      ...
                      <td style={{ padding: '10px 8px', textAlign: 'center' }}>
                        {hasStock ? (
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                            <button
                              ...
                              style={{
                                ...
                                borderRadius: 6,
                                border: 'none',
                                background: qty === 0 ? '#1e2130' : '#374151',
                                color: qty === 0 ? '#4b5563' : '#e2e8f0',
                          ...
                          <span style={{ color: '#6b7280', fontSize: '0.8rem' }}>Sold out</span>
                        )}
                      </td>
                    </tr>
                  );
        ...
        <div style={{ padding: '14px 24px', borderTop: '1px solid #2a2d3a', flexShrink: 0 }}>
          ...
          {box.discountPct > 0 && (
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: '#e97320', marginBottom: 4 }}>
              <span>
